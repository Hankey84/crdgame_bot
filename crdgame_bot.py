...
def start(message):
    ...
    item4 = telebot.types.KeyboardButton("Статистика")
    markup.row(item1)
    markup.row(item2)
    markup.row(item3)
    markup.row(item4)
    ...
    bot.send_message(user_id, "Для вызова статистики наберите: Statistics")
...
@bot.message_handler(
    ...
    in [
        "Уровень 1: Угадай цвет масти карты",
...
def start_game(user_id):
    ...
    user_stats.setdefault(
        user_id, {"correct": 0, "incorrect": 0}
    )  # Инициализация статистики

    if user_level[user_id] == 1:
        bot.send_message(user_id, "Угадай цвет масти (Red or Black):")
    elif user_level[user_id] == 2:
        bot.send_message(
            user_id, "Угадай масть карты:(Spades, Hearts, Clubs or Diamonds)"
        )
    elif user_level[user_id] == 3:
        bot.send_message(user_id, "Угадай карту: ")


# Функция для проверки угадывания
@bot.message_handler(
    func=lambda message: message.text
    in ["Red", "Black", "Spades", "Hearts", "Diamonds", "Clubs"] + ranks
)
def check_guess(message):
    ...
    if user_level[user_id] == 1:
        correct_suit_color = (
            "Red" if user_card[user_id]["suit"] in ["Hearts", "Diamonds"] else "Black"
        )
